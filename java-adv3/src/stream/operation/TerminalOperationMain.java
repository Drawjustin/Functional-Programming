...
public class TerminalOperationMain {
    public static void main(String[] args) {
        ...
        Integer[] arr = numbers.stream().filter(n -> n % 2 == 0)
                .toArray(Integer[]::new);
        ...
        long count = numbers.stream().filter( n -> n > 5)
                .count();
        ...
        System.out.println("짝수가 잇나? " + hasEven);

        System.out.println("12. allMatch - 모든 요소가 조건을 만족하는지");
        boolean allPositive = numbers.stream().anyMatch(n -> n > 0);
